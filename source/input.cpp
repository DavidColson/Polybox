...
void InputInit() {
	...
	i64 size;
	char* pData = ReadWholeFile("system/shared/base_controller_mapping.json", &size, g_pArenaFrame);
	if (pData == nullptr) {
    ...
    if (mapping.HasKey("Buttons")) {
        ...
        for (u64 i = 0; i < buttons.Count(); i++) {
            ...
            if (SubStr(altBindingLabel, 0, 5) == "Keyco") {
            ...
            } else if (SubStr(altBindingLabel, 0, 5) == "Mouse") {
    ...
    if (mapping.HasKey("Axes")) {
		...
		for (u64 i = 0; i < jsonAxes.Count(); i++) {
            ...
            if (jsonAxis.HasKey("Alt")) {
                ...
                if (SubStr(altBindingLabel, 0, 5) == "Scanc") {
                    SDL_Keycode keycode = pInputState->stringToKeyCode[Fnv1a::Hash(altBindingLabel.pData)];
